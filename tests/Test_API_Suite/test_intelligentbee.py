...
from sites.intelligentbee import intelligentbeeScraper
import pytest
import allure
import requests
...
def get_job_details():
    ...
    """
    scraper_data = intelligentbeeScraper().return_data()
    testutils = TestUtils()
...
@pytest.mark.regression
@pytest.mark.API
def test_intelligentbee_country_api(get_job_details):
...
@pytest.mark.regression
@pytest.mark.API
def test_intelligentbee_status_code_link_api(get_job_details):
