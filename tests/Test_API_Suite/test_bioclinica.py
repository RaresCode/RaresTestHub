...
from sites.bioclinica import bioclinicaScraper
import pytest
import allure
import requests
...
def get_job_details():
    ...
    """
    scraper_data = bioclinicaScraper().return_data()
    testutils = TestUtils()
...
@pytest.mark.regression
@pytest.mark.API
def test_bioclinica_link_api(get_job_details):
...
@pytest.mark.regression
@pytest.mark.API
def test_bioclinica_status_code_link_api(get_job_details):
