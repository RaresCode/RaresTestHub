...
from sites.inova import inovagroupScraper
import pytest
import allure
import requests
...
def get_job_details():
    ...
    """
    scraper_data = inovagroupScraper().return_data()
    testutils = TestUtils()
...
@pytest.mark.API
def test_inova_title_api(get_job_details):
...
@pytest.mark.regression
@pytest.mark.API
def test_inova_country_api(get_job_details):
...
@pytest.mark.regression
@pytest.mark.API
def test_inova_link_api(get_job_details):
...
@pytest.mark.regression
@pytest.mark.API
def test_inova_status_code_link_api(get_job_details):
